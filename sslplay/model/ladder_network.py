...
class ModelLadderNetwork:


    def __init__(self):
        self.name = "LADDER-NETWORK"
    ...
    def fit(self, X, y, Xu=None):
        ...
        if self.Xl.shape[0] > self.Xu.shape[0]:
            ...
            int_n_rep = int(self.Xu.shape[0] / self.Xl.shape[0]) + 1
            self.Xl = np.repeat(self.Xl, int_n_rep, axis=0)
            self.Xl = self.Xl[:self.Xu.shape[0],:]
            self.yl = np.repeat(self.yl, int_n_rep, axis=0)
